  ...
  let items = data?.map((product) => (
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        <button
          className="border py-1 w-[140px] font-[500] hover:bg-green-300 hover:text-white"
          onClick={() => dispatch({ type: "CART", payload: product })}
        >
          Add cart
        </button>
        <button className=" py-1 w-[140px] font-[500] text-red-500 border hover:bg-red-500 hover:text-white">
          Delete
        </button>
      </div>
    </div>
